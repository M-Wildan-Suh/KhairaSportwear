            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-900">
                        {{ number_format($todaySalesAmount + $todayRentalsAmount) }}</h3>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($recentTransactions as $transaction)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-4 py-3">
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">{{ $transaction->kode_transaksi }}
                                        </p>
                                        <p class="text-xs text-gray-500">{{ $transaction->created_at->format('d M H:i') }}
                                        </p>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                        ...
                        <div
                            ...
                            <div class="text-right">
                                <div class="flex items-center justify-end mb-1">
                                    <span class="text-xs font-medium text-red-600">{{ $product->stok_tersedia }}
                                        stok</span>
                                </div>
                                <div class="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
                                    <div class="h-full {{ $product->stok_tersedia == 0 ? 'bg-red-500' : 'bg-yellow-500' }}"
                                        style="width: {{ min(100, ($product->stok_tersedia / ($product->stok_awal ?: 1)) * 100) }}%">
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            @if ($activeRentals->count() > 0)
                <div class="space-y-3">
                    @foreach ($activeRentals->take(5) as $rental)
                        ...
                        @endphp
                        <div
                            class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-blue-50 transition-colors">
                            <div class="flex items-center">
                                @if ($rental->produk->gambar)
                                    <img src="{{ asset('storage/' . $rental->produk->gambar) }}"
                                        alt="{{ $rental->produk->nama }}" class="w-10 h-10 object-cover rounded-lg">
                                @else
                                    <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center">
                                        <i class="fas fa-box text-gray-400"></i>
                                    </div>
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-xs text-gray-500">
                                    {{ \Carbon\Carbon::parse($rental->tanggal_kembali_rencana)->format('d M') }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
