...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex items-center space-x-3">
            ...
            <div class="relative">
                ...
                <div id="filterDropdown" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-gray-200 z-10">
                    <div class="p-4 space-y-4">
                        <!-- Status Filter -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Status Sewa</label>
                            <select name="status" onchange="window.location.href='{{ route('admin.sewa.index') }}?status='+this.value" 
                        ...
                        <!-- Reset Button -->
                        <div>
                            <a href="{{ route('admin.sewa.index') }}" class="block w-full text-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">
                                Reset Filter
                            </a>
                        </div>
                    </div>
            ...
            <!-- View Active Rentals -->
            <a href="{{ route('admin.sewa.aktif') }}" class="px-4 py-2 border border-purple-500 text-purple-600 hover:bg-purple-50 rounded-lg flex items-center transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-rose-500 to-rose-600 flex items-center justify-center mr-3">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center mr-3">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100">
                <thead>
                    <tr class="bg-gray-50">
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">KODE SEWA</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($sewas as $sewa)
                        <tr class="hover:bg-blue-50/30 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-purple-100 to-purple-50 flex items-center justify-center mr-3">
                                        <i class="fas fa-calendar-alt text-purple-600"></i>
                                    </div>
                                    <div>
                                        <div class="text-sm font-semibold text-gray-900 font-mono">{{ $sewa->kode_sewa }}</div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold text-sm mr-3">
                                ...
                                <div class="flex items-center">
                                    @if($sewa->produk->gambar)
                                        <img class="w-10 h-10 rounded-lg object-cover border border-gray-200 mr-3" 
                                             src="{{ $sewa->produk->gambar_url }}" 
                                             alt="{{ $sewa->produk->nama }}">
                                    @else
                                        <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center border border-gray-200 mr-3">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @endphp
                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium {{ $statusColors[$sewa->status] ?? 'bg-gray-50 text-gray-700' }}">
                                ...
                                @if($sewa->status == 'aktif')
                                    @php
                                        ...
                                        $bgColor = $daysLeft <= 0 ? 'bg-rose-100 text-rose-700' : ($daysLeft < 3 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700');
                                        $icon = $daysLeft <= 0 ? 'fas fa-exclamation-triangle' : ($daysLeft < 3 ? 'fas fa-clock' : 'fas fa-calendar-check');
                                    @endphp
                                    <div class="mt-2">
