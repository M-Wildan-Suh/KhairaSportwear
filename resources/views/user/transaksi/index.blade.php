        ...
        <div class="container mx-auto px-4 lg:px-8">
            ...
            @if ($transaksis->count() > 0)
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                    <!-- Total Transaksi -->
                    <div class="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-300 hover:border-gray-800/30"
                        data-aos="fade-up">
                        <div class="flex items-center justify-between">
                            ...
                            <div
                                class="w-12 h-12 bg-gradient-to-br from-gray-800/10 to-gray-800/5 rounded-xl flex items-center justify-center">
                        ...
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <div class="flex justify-between text-sm">
                                ...
                                <span
                                    class="font-semibold text-green-600">{{ $transaksis->where('status', 'selesai')->count() }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Selesai -->
                    <div class="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-300 hover:border-green-200"
                        data-aos="fade-up" data-aos-delay="100">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-sm mb-2 font-medium">Selesai</p>
                                <h3 class="text-2xl font-bold text-gray-900">
                                    {{ $transaksis->where('status', 'selesai')->count() }}</h3>
                                <p class="text-xs text-gray-500 mt-1">Transaksi berhasil</p>
                            </div>
                            <div
                                class="w-12 h-12 bg-gradient-to-br from-green-100 to-green-50 rounded-xl flex items-center justify-center">
                                <i class="fas fa-check-circle text-green-600 text-lg"></i>
                            </div>
                        </div>
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <div
                                class="text-xs text-green-600 bg-green-50 px-3 py-1 rounded-full inline-flex items-center gap-1">
                    ...
                    <!-- Dalam Proses -->
                    <div class="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-300 hover:border-yellow-200"
                        data-aos="fade-up" data-aos-delay="200">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-sm mb-2 font-medium">Dalam Proses</p>
                                <h3 class="text-2xl font-bold text-gray-900">
                                    {{ $transaksis->whereIn('status', ['pending', 'diproses'])->count() }}</h3>
                                <p class="text-xs text-gray-500 mt-1">Menunggu konfirmasi</p>
                            </div>
                            <div
                                class="w-12 h-12 bg-gradient-to-br from-yellow-100 to-yellow-50 rounded-xl flex items-center justify-center">
                    ...
                    <!-- Dibatalkan -->
                    <div class="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-300 hover:border-red-200"
                        data-aos="fade-up" data-aos-delay="300">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-gray-600 text-sm mb-2 font-medium">Dibatalkan</p>
                                <h3 class="text-2xl font-bold text-gray-900">
                                    {{ $transaksis->where('status', 'dibatalkan')->count() }}</h3>
                                <p class="text-xs text-gray-500 mt-1">Transaksi dibatalkan</p>
                            </div>
                            <div
                                class="w-12 h-12 bg-gradient-to-br from-red-100 to-red-50 rounded-xl flex items-center justify-center">
                                <i class="fas fa-times-circle text-red-600 text-lg"></i>
                            </div>
                        </div>
                        <div class="mt-4 pt-4 border-t border-gray-100">
                            <div
                                class="text-xs text-red-600 bg-red-50 px-3 py-1 rounded-full inline-flex items-center gap-1">
                                <i class="fas fa-chart-line"></i>
                                <span>{{ $transaksis->total() > 0 ? round(($transaksis->where('status', 'dibatalkan')->count() / $transaksis->total()) * 100, 1) : 0 }}%
                                    dari total</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-600 mt-1">Menampilkan {{ $transaksis->count() }} dari
                                    {{ $transaksis->total() }} transaksi</p>
                            </div>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="bg-gray-50/80">
                                    <th
                                        class="py-4 px-6 text-left text-xs font-semibold text-gray-900 uppercase tracking-wider">
                                        Kode Transaksi</th>
                                    <th
                                        class="py-4 px-6 text-left text-xs font-semibold text-gray-900 uppercase tracking-wider">
                                        Tanggal</th>
                                    ...
                                    <tr class="hover:bg-gray-50/50 transition-colors duration-200 group"
                                        ...
                                        <td class="py-5 px-6">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-10 h-10 rounded-lg bg-gray-800/10 flex items-center justify-center group-hover:bg-gray-800/20 transition-colors">
                                                    <i class="fas fa-receipt text-gray-800 text-sm"></i>
                                                </div>
                                                <div>
                                                    <div
                                                        class="font-semibold text-gray-900 group-hover:text-gray-800 transition-colors">
                                                        {{ $transaksi->kode_transaksi }}
                                                    </div>
                                                    <div class="text-xs text-gray-500 mt-0.5">
                                                        {{ $transaksi->detailTransaksis->count() }} item</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="py-5 px-6">
                                            @if ($transaksi->tipe === 'penjualan')
                                                <div
                                                    class="inline-flex items-center gap-2 px-3 py-1.5 bg-green-50 text-green-700 rounded-full border border-green-100">
                                        ...
                                        <td class="py-5 px-6">
                                            <div class="space-y-2">
                                                @php
                                                    ...
                                                    $config =
                                                        $statusConfig[$transaksi->status] ?? $statusConfig['pending'];
                                                @endphp
            ...
            rounded-lg border border-{{ $config['color'] }}-100">
                                                    ...
                                                    <span
                                                        class="text-sm font-medium capitalize">{{ $transaksi->status }}</span>
                                                </div>

                                                {{-- Info Pending --}}
                                                @if ($transaksi->status === 'pending')
                                                    <div
                                                        class="flex items-center gap-1 text-xs text-red-600 bg-red-50 px-2 py-1 rounded">
                                                        <i class="fas fa-exclamation-circle"></i>
                                                        <span>
                                                            {{ $transaksi->metode_pembayaran == 'tunai' ? 'Lunasi produk di toko' : 'Upload bukti pembayaran' }}
                                                        </span>
                                                    </div>
                                                @endif

                                                {{-- Tombol Verifikasi --}}
                                                @if ($transaksi->status === 'selesai' && !$transaksi->verified_at)
                                                    <form action="{{ route('user.transaksi.verify', $transaksi->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        <button type="submit"
                                                            class="inline-flex items-center gap-2 text-xs font-semibold text-green-700 bg-green-100 hover:bg-green-200 px-3 py-1.5 rounded-lg transition">
                                                            <i class="fas fa-shield-check"></i>
                                                            Verifikasi Transaksi
                                                        </button>
                                                    </form>
                                                @elseif ($transaksi->verified_at)
                                                    <button type="submit"
                                                        class="flex items-center gap-2 text-xs font-semibold text-green-700 bg-green-200 px-3 py-1.5 rounded-lg transition">
                                                        <i class="fas fa-shield-check"></i>
                                                        Sudah di verifikasi
                                                    </button>
                                                @endif
                                        ...
                                        <td class="py-5 px-6">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <a href="{{ route('user.transaksi.show', $transaksi->id) }}"
                                                    ...
                                                    title="Lihat Detail">
                                                    <i
                                                        class="fas fa-eye group-hover/btn:scale-110 transition-transform"></i>
                                                </a>
                                                ...
                                                @if ($transaksi->status === 'pending' && $transaksi->metode_pembayaran != 'tunai')
                                                    <button onclick="showUploadModal('{{ $transaksi->id }}')"
                                                        ...
                                                        title="Upload Bukti">
                                                        <i
                                                            class="fas fa-upload group-hover/btn:scale-110 transition-transform"></i>
                                                    </button>
                                                @endif
                                                ...
                                                @if (in_array($transaksi->status, ['pending', 'diproses']))
                                                    <button onclick="cancelTransaction('{{ $transaksi->id }}')"
                                                        ...
                                                        <i
                                                            class="fas fa-times group-hover/btn:scale-110 transition-transform"></i>
                                                    </button>
                                                @endif
                                            </div>
                                        </td>
            ...
            @else
                ...
                <div class="max-w-md mx-auto text-center py-16" data-aos="fade-up">
                    <div class="relative mb-8">
                        <div
                            class="w-32 h-32 mx-auto bg-gradient-to-br from-gray-100 to-gray-50 rounded-full flex items-center justify-center">
                            <i class="fas fa-receipt text-gray-400 text-5xl"></i>
                        </div>
                        <div
                            class="absolute -top-2 -right-2 w-12 h-12 bg-gray-800/10 rounded-full flex items-center justify-center">
                            <i class="fas fa-search text-gray-800"></i>
                        </div>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-900 mb-3">Belum Ada Transaksi</h3>
                    <p class="text-gray-600 mb-8">Mulai berbelanja untuk melihat riwayat transaksi Anda. Transaksi Anda
                ...
                <form id="uploadForm" enctype="multipart/form-data" class="p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <div id="uploadArea"
                                ...
                                <input id="fileInput" name="bukti_pembayaran" type="file"
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <h4 class="font-semibold text-blue-900 mb-2">Pastikan bukti transfer jelas menunjukkan:
                                </h4>
...
@push('scripts')
    ...
    `;
        ...
        async function cancelTransaction(transaksiId) {
            if (!await showConfirmDialog('Batalkan Transaksi',
                    'Apakah Anda yakin ingin membatalkan transaksi ini? Stok akan dikembalikan.', 'warning')) {
                return;
            }
        ...
        function showToast(type, message, autoClose = true) {
            ...
            toast.id = toastId;
            toast.className =
                `fixed top-6 right-6 px-6 py-4 rounded-xl shadow-lg z-50 transform transition-all duration-300 translate-x-full ${colors[type]} text-white border`;
            toast.innerHTML = `
        <div class="flex items-center gap-3">
    ...
    `;
        ...
        `;
                ...
                // Animate in
                setTimeout(() => {
                    document.getElementById(`${dialogId}-content`).classList.remove('scale-95',
                        'opacity-0');
                    document.getElementById(`${dialogId}-content`).classList.add('scale-100',
                        'opacity-100');
                }, 10);
