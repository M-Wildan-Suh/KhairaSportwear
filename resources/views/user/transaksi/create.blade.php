            ...
            <form id="checkoutForm" method="POST" action="{{ route('user.transaksi.store') }}">
                ...
                <div class="grid lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2">
                                            ...
                                            @foreach ($keranjangs as $item)
                                                <tr class="border-b border-gray-100 last:border-0">
                                                    ...
                                                    <td class="py-4 text-center">
                                                        @if ($item->tipe === 'jual')
                                                            <span
                                                                class="px-3 py-1 bg-green-100 text-green-800 text-sm font-medium rounded-full">Beli</span>
                                                        @else
                                                            <span
                                                                class="px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full">Sewa</span>
                                                        @endif
                                                    </td>
                                                    <td class="py-4 text-center">
                            ...
                            <div class="bg-white rounded-xl border border-gray-200 shadow-sm mb-6" data-aos="fade-right"
                                ...
                                <div class="p-6">
                                    <div>
                                        ...
                                        <p class="mt-2 text-sm text-gray-500">Pastikan alamat lengkap dan jelas untuk
                                            pengiriman</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Payment Method -->
                        <div class="bg-white rounded-xl border border-gray-200 shadow-sm" data-aos="fade-right"
                            data-aos-delay="200">
                            <div class="p-6 border-b border-gray-200">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center">
                            ...
                            <div class="p-6">
                                ...
                                <div class="space-y-4 mb-6">
                                    <label
                                        class="flex items-center p-4 border border-gray-300 rounded-lg hover:border-primary hover:bg-primary/5 cursor-pointer transition-all">
                                        <input type="radio" name="metode_pembayaran" value="transfer_bank"
                                            class="h-5 w-5 text-primary focus:ring-primary" checked>
                                        <div class="ml-4">
                                            <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                @if (is_array($bankInfo))
                                                    @foreach ($bankInfo as $bank)
                                                        <div class="text-blue-700">{{ $bank }}:
                                                            {{ $noRekening }} a/n {{ $namaRekening }}</div>
                                                    @endforeach
                                                @else
                                                    <div class="text-blue-700">{{ $bankInfo }}: {{ $noRekening }}
                                                        a/n {{ $namaRekening }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="mb-6">
                                    ...
                                    <textarea name="catatan" rows="3" placeholder="Catatan untuk penjual..."
    ...
    <div id="loadingModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl max-w-sm w-full p-8 text-center">
                <div class="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto mb-6">
                </div>
...
<script>
    window.hasItemJual = {{ $keranjangs->where('tipe', 'jual')->isNotEmpty() ? 'true' : 'false' }};
</script>
...
@push('scripts')
    <script>
        ...
        const cashRadio = document.querySelector('input[value="tunai"]');
        const cashLabel = cashRadio?.closest('label');

        // 🔥 BLOKIR TUNAI JIKA ADA ITEM JUAL
        if (window.hasItemJual && cashRadio) {
            cashRadio.disabled = true;
            cashRadio.checked = false;

            if (cashLabel) {
                cashLabel.classList.add('opacity-50', 'cursor-not-allowed');
                cashLabel.title = 'Pembayaran tunai tidak tersedia untuk produk jual';
            }

            // pastikan metode lain terpilih
            document.querySelector('input[value="transfer_bank"]')?.click();
        }
