            ...
            @if ($sewas->count() > 0)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                    @foreach ($sewas as $sewa)
                        ...
                        <div class="bg-white rounded-xl border {{ $isDendaActive && $sewa->status == 'aktif' ? 'border-red-500 shadow-red-100' : ($isOverdue && $sewa->status == 'aktif' ? 'border-orange-500' : 'border-gray-200') }} shadow-sm hover:shadow-md transition-all duration-300"
                            data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">

                            <!-- Peringatan Denda - Muncul jika telat >= 3 hari -->
                            @if ($isDendaActive && $sewa->status == 'aktif')
                                <div class="bg-red-50 border-b border-red-200 px-6 py-4">
                                    <div class="flex items-start gap-3">
                                        <div class="flex-shrink-0">
                                            <div
                                                class="w-10 h-10 bg-red-600 rounded-full flex items-center justify-center animate-pulse">
                            ...
                            @elseif($isOverdue && $sewa->status == 'aktif')
                                <!-- Peringatan Terlambat - Kurang dari 3 hari (TANPA DENDA) -->
                                <div class="bg-orange-50 border-b border-orange-200 px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <i class="fas fa-clock text-orange-600"></i>
                                        <p class="text-sm font-medium text-orange-900">
                                            Terlambat {{ $hariTerlambat }} hari (denda mulai dihitung hari ke-3)
                                        </p>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex gap-4 mb-6">
                                    <img src="{{ $sewa->produk->gambar_url }}" alt="{{ $sewa->produk->nama }}"
                                        class="w-20 h-20 object-cover rounded-lg">
                                    <div class="flex-1">
                                        <div class="grid grid-cols-2 gap-3">
                                            <div class="flex items-center gap-2">
                                                <div
                                                    class="w-8 h-8 bg-gray-800/10 rounded-lg flex items-center justify-center">
                                                    <i class="fas fa-calendar-alt text-gray-800 text-sm"></i>
                                                </div>
                                                <div>
                                                    <p class="text-xs text-gray-600">Durasi</p>
                                                    <p class="text-sm font-medium">{{ $sewa->durasi }}
                                                        ({{ $sewa->jumlah_hari }} hari)
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm font-medium">
                                                        {{ $sewa->tanggal_mulai->format('d/m/Y') }}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm font-medium">
                                                        {{ $sewa->tanggal_kembali_rencana->format('d/m/Y') }}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-sm font-medium">Rp
                                                        {{ number_format($sewa->total_harga, 0, ',', '.') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="mb-6">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        @elseif ($sewa->status == 'selesai')
                                            <span class="text-sm font-medium text-green-500">Sudah Dikembalikan</span>
                                        @else
                                            <span
                                                class="text-sm font-medium {{ $sisaHari < 3 ? 'text-orange-600' : 'text-green-600' }}">
                                    ...
                                    <div class="relative">
                                        @php
                                            ...
                                            $percentage =
                                                $totalDays > 0 ? min(100, ($elapsedDays / $totalDays) * 100) : 0;
                                        ...
                                        <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                                            <div class="h-full {{ $isOverdue && $sewa->status == 'aktif' ? 'bg-gradient-to-r from-red-600 to-red-800' : 'bg-gradient-to-r from-gray-800 to-green-500' }} rounded-full transition-all duration-500"
                                        ...
                                        <div class="absolute inset-0 flex justify-between items-center px-1">
                                            @for ($i = 0; $i <= $totalDays; $i++)
                                                <div
                                                    class="w-px h-3 {{ $i <= $elapsedDays ? ($isOverdue && $sewa->status == 'aktif' ? 'bg-red-600' : 'bg-gray-800') : 'bg-gray-300' }}">
                                                </div>
                                            @endfor
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex justify-between text-xs text-gray-500 mt-1">
                                        <span>Mulai</span>
                                        <span
                                            class="{{ $isOverdue && $sewa->status == 'aktif' ? 'text-red-600 font-bold' : '' }}">
                                            {{ $isOverdue && $sewa->status == 'aktif' ? 'Lewat ' . $hariTerlambat . ' hari' : 'Kembali' }}
                                        </span>
                                    </div>
                                </div>

                                <!-- Informasi Denda (hanya tampil jika telat >= 3 hari) -->
                                @if ($isDendaActive && $sewa->status == 'aktif')
                                    <div class="mb-4 bg-orange-50 border border-orange-200 rounded-lg p-4">
                                        <div class="flex items-start gap-3">
                                            <i class="fas fa-calculator text-orange-600 mt-0.5"></i>
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-orange-900 mb-2">Perhitungan Denda
                                                    Keterlambatan:</p>
                                                ...
                                                <div class="space-y-1 text-sm text-orange-800">
                                                    <div class="flex justify-between">
                                                        ...
                                                        <span class="font-medium">Rp
                                                            {{ number_format($tarifDendaPerHari, 0, ',', '.') }}</span>
                                                    </div>

                                                    <div class="flex justify-between">
                                                        <span>Keterlambatan total:</span>
                                                        <span class="font-medium">{{ $hariTerlambat }} hari</span>
                                                    </div>

                                                    <div class="flex justify-between">
                                                        <span>Hari kena denda (mulai hari ke-3):</span>
                                                        <span class="font-medium">{{ $hariKenaDenda }} hari</span>
                                                    </div>

                                                    <div class="flex justify-between pt-2 border-t border-orange-300">
                                                        <span class="font-bold">Estimasi Total:</span>
                                                        <span class="font-bold text-orange-900">Rp
                                                            {{ number_format($estimasiDenda, 0, ',', '.') }}</span>
                                                    </div>
                                                </div>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ route('user.sewa.struk', ['struk' => $sewa->id]) }}" target="_blank"
                                    ...
                                    <button onclick="showReturnModal('{{ $sewa->id }}')"
                                        {{ $sewa->status == 'aktif' ? '' : 'disable' }}
                                        class="{{ $isOverdue && $sewa->status == 'aktif' ? 'bg-red-600 hover:bg-red-700' : 'bg-green-600 hover:bg-green-700' }} {{ $sewa->status === 'aktif' ? '' : 'opacity-50 cursor-not-allowed' }} flex-1 min-w-[120px] px-4 py-2 text-white font-medium rounded-lg transition-colors">
                                        <i class="fas fa-undo mr-2"></i>
                                        {{ $isOverdue && $sewa->status == 'aktif' ? 'Kembalikan SEGERA!' : 'Kembalikan' }}
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid lg:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="date" name="tanggal_kembali" id="return_tanggal_kembali"
                                ...
                                <div>
                                    ...
                                    <select name="kondisi_alat" id="return_kondisi_alat"
                                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-800 focus:border-gray-800"
                                        required>
                                ...
                                <div>
                                    ...
                                    <textarea name="catatan_kondisi" rows="3" placeholder="Deskripsi kondisi alat..."
                                ...
                                <div class="bg-gray-50 rounded-lg p-6">
                                    ...
                                    <div id="fineCalculation" class="space-y-3">
                                        <div class="text-center py-8">
                                            <p class="text-gray-600">Pilih tanggal kembali dan kondisi alat untuk
                                                menghitung denda</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div class="flex items-start">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-blue-700">Pengembalian akan diverifikasi oleh admin. Denda akan
                                        dikenakan jika terdapat keterlambatan atau kerusakan.</p>
                                </div>
                            </div>
                        </div>
            ...
            <div class="relative bg-white rounded-xl max-w-md w-full p-6">
                ...
                <form id="extendForm" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="tambahan_hari" min="1" max="30" value="1"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-800 focus:border-gray-800"
                            required>
                    ...
                    <div>
                        ...
                        <textarea name="alasan" rows="2" placeholder="Alasan perpanjangan sewa..."
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-800 focus:border-gray-800"></textarea>
                    </div>
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-yellow-700">Perpanjangan sewa akan menambah biaya sesuai durasi
                                    tambahan.</p>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    ...
    `;
            ...
            try {
                ...
                if (!result.success) {
                    throw new Error(result.message || 'Gagal menghitung denda');
                }
            ...
            `;
                    ...
                    <div class="flex justify-between items-center">
                        <span class="text-gray-600">Perhitungan Denda:</span>
                        <span class="font-medium">
                            ${fines.hari_kena_denda} hari × ${fines.formatted.tarif_denda_per_hari}
                        </span>
                    </div>
                ...
                `;
                    ...
                    // DENDA KERUSAKAN
                    if (fines.denda_kerusakan > 0) {
                        fineHtml += `
                    <div class="flex justify-between items-center">
                        <span class="text-gray-600">Denda Kerusakan:</span>
                        <span class="text-red-600 font-medium">
                            ${fines.formatted.denda_kerusakan}
                        </span>
                    </div>
                `;
                    }
            ...
            <div class="text-center py-4">
                ...
                <p class="text-red-600 font-medium">${error.message || 'Terjadi kesalahan jaringan'}</p>
            </div>
        `;
            }
        }
        ...
        function showToast(type, message) {
            ...
            toast.className =
                `fixed top-4 right-4 px-6 py-3 rounded-lg shadow-lg z-50 animate-slide-in-right ${type == 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`;
