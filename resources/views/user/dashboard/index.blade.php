...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <!-- Welcome Banner -->
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-primary to-secondary mb-8 p-8 text-white"
            data-aos="fade-down">
            <div class="flex flex-col md:flex-row items-center">
                <div class="md:w-2/3">
                    <h1 class="text-3xl md:text-4xl font-bold mb-4 text-black">Selamat Datang, <span
                            class="text-yellow-300">{{ $user->name }}</span>! 👋</h1>
                    <p class="text-lg opacity-90 mb-0 text-black">Apa yang ingin Anda lakukan hari ini? Mari mulai perjalanan
                        olahraga Anda!</p>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-primary transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                data-aos="fade-up" data-aos-delay="100">
                <div class="flex items-center">
                    <div
                        class="bg-primary bg-opacity-10 rounded-xl p-3 mr-4 transition-all duration-300 group-hover:scale-110">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-green-500 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
            ...
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-blue-500 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
            ...
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-yellow-500 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                    ...
                    <div class="flex flex-col md:flex-row items-center justify-between">
                        <div class="flex items-center mb-4 md:mb-0">
                            ...
                            <div>
                                ...
                                <p class="opacity-90">Anda memiliki {{ $lateRentals->count() }} sewa yang melewati batas
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {{-- Sewa Bulan Ini --}}
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-purple-500" data-aos="fade-up"
                data-aos-delay="100">
                <div class="flex items-center">
            ...
            {{-- Total Denda --}}
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-red-500" data-aos="fade-up"
                data-aos-delay="200">
                <div class="flex items-center">
            ...
            {{-- Menunggu Verifikasi --}}
            <div class="bg-white rounded-2xl p-6 shadow-md border-l-4 border-orange-500" data-aos="fade-up"
                data-aos-delay="300">
                <div class="flex items-center">
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors">
                                                <td class="px-4 py-3">
                                                    ...
                                                    <div class="text-sm text-gray-500">
                                                        {{ $transaction->tipe == 'penjualan' ? 'Pembelian' : 'Penyewaan' }}
                                                    </div>
                                                </td>
                                                <td class="px-4 py-3 whitespace-nowrap">
                                                    {{ $transaction->created_at->format('d/m/Y') }}</td>
                                                <td class="px-4 py-3 whitespace-nowrap font-bold">Rp
                                                ...
                                                <td class="px-4 py-3 whitespace-nowrap">
                                                    <div class="flex flex-col gap-1">
                                                        ...
                                                        @if ($transaction->status === 'selesai' && !is_null($transaction->verified_at))
                                                            <span
                        ...
                        @if ($activeRentalsList->count() > 0)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach ($activeRentalsList as $rental)
                                    <div
                                        class="border rounded-xl p-4 hover:border-primary hover:shadow-md transition-all duration-300">
                                        ...
                                        <div class="space-y-2 mb-4">
                                            <div class="flex justify-between">
                                                ...
                                                <span
                                                    class="text-sm font-medium {{ $rental->sisa_hari < 3 ? 'text-red-500' : 'text-green-500' }}">
                                        ...
                                        <div class="flex gap-2">
                                            ...
                                            <button
                                                class="btn-success-outline text-sm py-2 flex items-center justify-center"
                    ...
                    <div class="px-6 pt-6">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            @if ($notifications->count() > 0)
                                <form action="{{ route('user.notifikasi.mark-all-read') }}" method="POST"
                                    class="inline">
                                    @csrf
                                        ...
                                        <div class="flex">
                                            ...
                                            <div class="flex-1">
                                                <div class="flex justify-between items-start">
                                                    <h6
                                                        class="{{ $notification->dibaca ? 'text-gray-700' : 'font-bold text-gray-900' }} mb-1">
                                                        {{ $notification->judul }}
                                                    </h6>
                                                    <small
                                                        class="text-gray-500 text-xs">{{ $notification->created_at->diffForHumans() }}</small>
                                                </div>
                                                <p class="text-gray-600 text-sm">
                                                    {{ Str::limit($notification->pesan, 50) }}</p>
                                            </div>
                                        </div>
                                    </a>
                                @endforeach
                        ...
                        <div class="px-6 pb-6">
                            <div class="space-y-3">
                                @foreach ($lateRentals as $rental)
                                    <div
                                        class="flex justify-between items-center py-3 border-b border-red-100 last:border-0">
                                        <div class="flex-1">
                                        ...
                                        <div class="text-right">
                                            <span class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-xs font-bold">
                                                Rp
                                                {{ number_format($rental->total_harga * 0.1 * $rental->hitungKeterlambatan(), 0, ',', '.') }}
                                            </span>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <div class="px-6 pb-6">
                            <div class="space-y-3">
                                @foreach ($pendingReturns as $rental)
                                    <div
                                        class="flex justify-between items-center py-3 border-b border-orange-100 last:border-0">
                                        <div>
                                            <h6 class="text-sm font-medium text-gray-800">{{ $rental->produk->nama }}</h6>
                                            <p class="text-xs text-orange-500">
                                                <i class="fas fa-calendar-check mr-1"></i>
                                                Dikembalikan
                                                {{ $rental->tanggal_kembali_aktual ? $rental->tanggal_kembali_aktual->format('d/m') : '-' }}
                                            </p>
                                        </div>
                                        <span
                            ...
                            <div class="mt-4">
                                <a href="{{ route('user.sewa.menunggu-verifikasi') }}"
                    ...
                    <div class="px-6 pb-6">
                        <div class="grid grid-cols-2 gap-4">
                            <a href="{{ route('user.keranjang.index') }}"
                                ...
                                @if ($cartCount > 0)
                                    <span
                                        class="mt-1 bg-accent text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">
                                        {{ $cartCount }}
                                    </span>
                                @endif
                            </a>
                                    ...
                                    <div
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">Kembali:
                                                {{ $rental->tanggal_kembali_rencana->format('d/m') }}</p>
                                        </div>
                                        <span
                                            class="px-3 py-1 rounded-full text-xs font-medium {{ $rental->sisa_hari < 2 ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800' }}">
                                            {{ $rental->sisa_hari }} hari
                                        </span>
                                    </div>
                                @endforeach
                ...
                fetch(`/user/notifications/${notificationId}/read`, {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            notificationElement.querySelector('h6').classList.remove('font-bold',
                                'text-gray-900');
