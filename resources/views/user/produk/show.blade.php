        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <i class="fas fa-chevron-right text-gray-400"></i>
                        <a href="{{ route('produk.kategori', $produk->kategori->slug) }}" class="ml-2 text-gray-600 hover:text-primary">{{ $produk->kategori->nama }}</a>
                    </div>
                </li>
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            <label class="color-option cursor-pointer">
                                <input type="radio" name="selected_warna" value="{{ $warna }}" 
                                       class="sr-only" 
                                       {{ $loop->first ? 'checked' : '' }}>
                                <div class="flex items-center gap-2 px-4 py-2.5 border-2 border-gray-200 rounded-lg hover:border-primary transition-all duration-200">
                                    <div class="w-5 h-5 rounded-full border border-gray-300" 
                                         style="background-color: {{ \App\Models\Produk::getColorCode($warna) }}"></div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="size-option">
                                <input type="radio" name="selected_size" value="{{ $size }}" 
                                       class="sr-only" 
                                       {{ $loop->first ? 'checked' : '' }}>
                                <div class="px-5 py-2.5 border-2 border-gray-200 rounded-lg text-center font-medium text-gray-700 hover:border-primary hover:text-primary hover:bg-primary/5 cursor-pointer transition-all duration-200">
                                    {{ strtoupper($size) }}
                                </div>
                            </label>
                            @endforeach
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-600">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            ...
                                            <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                                <div class="w-3 h-3 rounded-full" style="background-color: {{ \App\Models\Produk::getColorCode($warna) }}"></div>
                                                {{ ucfirst($warna) }}
                                            </span>
                                            @endforeach
                                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="border border-gray-200 rounded-lg overflow-hidden">
                            ...
                            <div class="faq-answer px-6 py-4 hidden">
                                <p class="text-gray-600">Ya, semua warna dan ukuran yang tercantum di halaman produk tersedia. Jika suatu warna atau ukuran habis, akan kami tandai sebagai "Habis".</p>
                            </div>
                        </div>
    ...
    <div class="container">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div data-aos="fade-up" data-aos-delay="{{ $loop->index * 100 }}">
                <a href="{{ route('produk.show', $related->slug) }}" class="group block">
                    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                        ...
                        <div class="p-4">
                            ...
                            @if($related->warna && count($related->warna) > 0)
                            <div class="flex items-center gap-1 mb-2">
                                @foreach(array_slice($related->warna, 0, 3) as $warna)
                                <div class="w-4 h-4 rounded-full border border-gray-300" 
                                     style="background-color: {{ \App\Models\Produk::getColorCode($warna) }}"
                                     title="{{ ucfirst($warna) }}"></div>
                                @endforeach
                                @if(count($related->warna) > 3)
                                <span class="text-xs text-gray-500">+{{ count($related->warna) - 3 }}</span>
                                @endif
                            </div>
                            @endif
                            <div class="flex items-center justify-between">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <label class="modal-color-option">
                                    <input type="radio" name="warna" value="{{ $warna }}" 
                                           class="sr-only" 
                                           {{ $loop->first ? 'checked' : '' }}>
                                    <div class="flex items-center gap-2 px-3 py-1.5 border border-gray-300 rounded-lg hover:border-primary cursor-pointer">
                                        <div class="w-4 h-4 rounded-full border border-gray-300" 
                                             style="background-color: {{ \App\Models\Produk::getColorCode($warna) }}"></div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <label class="modal-size-option">
                                    <input type="radio" name="size" value="{{ $size }}" 
                                           class="sr-only" 
                                           {{ $loop->first ? 'checked' : '' }}>
                                    <div class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:border-primary hover:text-primary hover:bg-primary/5 cursor-pointer">
                                        {{ strtoupper($size) }}
                                    </div>
                                </label>
                                @endforeach
...
function updateSelectedColorSizeDisplay() {
    ...
    if (selectedWarnaRadio) {
        ...
        if (selectedWarnaEl) {
            selectedWarnaEl.textContent = selectedWarnaRadio.value.charAt(0).toUpperCase() + selectedWarnaRadio.value.slice(1);
        }
    }
